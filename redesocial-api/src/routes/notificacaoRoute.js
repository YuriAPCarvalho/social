...
router.get("/:id", verificar, async (req, res) => {
    try {
        const resposta = await
            notificacaoService.buscarPorId(req.params.id)
        res.json(resposta)
...
router.get("/perfil/:id", verificar, async (req, res) => {
    try {
        const resposta = await
            notificacaoService.buscarPorPerfilId(req.params.id)
        res.json(resposta)
...
router.post("", verificar, async (req, res) => {
    try {
        const resposta = await
            notificacaoService.cadastrar(req.body)
        res.json(resposta)
...
router.put("/lida/:id", verificar, async (req, res) => {
    try {
        const resposta = await
            notificacaoService.marcarLida(req.params.id)
        res.json(resposta)
